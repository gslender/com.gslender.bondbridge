...
class BondDevice extends Device {
  ...
  async runBondAction(action, args = {}) {
    return this.bond.sendBondAction(this.getData().id, action, args);
  }
  ...
  async safeUpdateCapabilityValue(key, value) {
    this.log(`safeUpdateCapabilityValue '${this.getData().id}' '${key}' '${value}'`);
